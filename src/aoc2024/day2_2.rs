...
fn is_safe(line: &str) -> bool {
    ...
    for i in 0..len - 1 {
        ...
        if d.abs() > 3
            || d == 0
            || (is_positive && d.is_negative())
            || (!is_positive && d.is_positive())
        {
            for j in 0..len {
                let mut option = entries.clone();
                option.remove(j);
                if inner_is_safe(&option) {
                    return true;
                }
            }
            return false;
...
fn inner_is_safe(input: &[i32]) -> bool {
    if match input[0].cmp(&input[1]) {
        std::cmp::Ordering::Less => input.windows(2).any(|pair| pair[0] >= pair[1]),
        std::cmp::Ordering::Equal => false,
        std::cmp::Ordering::Greater => input.windows(2).any(|pair| pair[0] <= pair[1]),
    } {
        return false;
    }
    !input
